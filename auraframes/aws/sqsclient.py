...
class SQSClient(AWSClient):
    ...
    def get_queue_url(self, frame_id: str) -> str | None:
        result: str | None = self.sqs_client.get_queue_url(
            QueueName=f"frame-{frame_id}-client"
        ).get("QueueUrl")
        return result

    def receive_message(
        self, queue_url: str, max_num_messages: int = 10, wait_time_seconds: int = 20
    ) -> dict[str, Any]:
