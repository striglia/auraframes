...
class FrameApi(BaseApi):
    ...
    def get_frame(self, frame_id: str) -> tuple[Frame, int]:
        ...
        return Frame(**json_response.get("frame")), json_response.get(
            "total_asset_count"
        )
    ...
    def get_assets(
        ...
        To obtain the next set of pages, pass in the cursor from the response.
    ...
    def get_activities(
        ...
        """
        json_response = self._client.get(
            f"/frames/{frame_id}/activities.json", query_params={"cursor": cursor}
        )
        return (
            [
                Activity(**json_activity)
                for json_activity in json_response.get("activities")
            ],
            json_response.get("next_page_cursor"),
        )

    def show_asset(self, frame_id: str, asset_id: str, goto_time: str | None) -> bool:
        """
        Forces the frame to display the asset.

        :param frame_id: Frame id to control
        :param asset_id: Asset id to display on the frame
        :param goto_time: TODO: Unknown, appears to be the current datetime
    ...
    def select_asset(self, frame_id: str, asset_partial_id: AssetPartialId) -> int:
        ...
        return int(json_response.get("number_failed"))

    def exclude_asset(self, frame_id: str, asset_partial_id: AssetPartialId) -> int:
        """
        Excludes an asset from displaying in the frame's slideshow.
        ...
        # Typical use of this endpoint results in a single AssetPartialId being sent per call.
        json_response = self._client.post(
            f"/frames/{frame_id}/exclude_asset",
            data={"assets": [asset_partial_id.to_request_format()]},
        )
    ...
    def remove_asset(self, frame_id: str, asset_partial_id: AssetPartialId) -> int:
        """
        Disassociates an asset from a frame.
        ...
        # Typical use of this endpoint results in a single AssetPartialId being sent per call.
        json_response = self._client.post(
            f"/frames/{frame_id}/remove_asset.json",
            data={"assets": [asset_partial_id.to_request_format()]},
        )
